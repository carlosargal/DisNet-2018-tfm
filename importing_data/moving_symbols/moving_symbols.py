...
from moving_symbols_utils import merge_dicts, tight_crop, compute_pm_hull_vertices, create_sine_fn, create_triangle_fn

_COLLISION_TYPES = dict(symbol=0, wall=1)
...
class ImageLoader:
    ...
    def __init__(self, root, mode=None):
        ...
        Args:
            ...
            mode: String that indicates how to transform the image for rendering. Options
...
class Symbol:
    ...
    def get_render_image_and_position(self, screen_size):
        ...
        """
        # Get scaled image
        scaled_image = pg.transform.smoothscale(self.pg_image, tuple([int(x * self.scale) for x in self.image.size]))
...
class MovingSymbolsEnvironment:
    ...
    * **hit_wall**
    ...
    DEFAULT_PARAMS = dict(data_dir='../data/mnist',
                          split='training',
                          num_symbols=1,
                          video_size=(64, 64),
                          color_output=True,
    ...
    DEFAULT_DEBUG_OPTIONS = dict(frame_number_font_size=30,
    ...
    def __init__(self, params, seed, fidelity=10, debug_options=None):
        ...
        self._add_init_message(dict(step=-1,
                                    type='params',
        ...
        self.symbols = []
        image_loader = ImageLoader(os.path.join(self.params['data_dir'], self.params['split']), 'tight_crop')
        ...
        for id in xrange(self.params['num_symbols']):
            label = self.params['symbol_labels'][np.random.randint(len(self.params['symbol_labels']))]
            ...
            period = np.random.uniform(*tuple(self.params['scale_period_limits'][period_limits_index]))
            ...
            if self.params['scale_function_type'] == 'sine':
                ...
                scale_fn = create_triangle_fn(period, y_offset, self.params['movement'])
            elif self.params['scale_function_type'] == 'constant':
                scale = np.random.uniform(*self.params['scale_limits'])
                scale_fn = lambda x: scale
            else:
                raise ValueError('scale_function_type "%s" is unsupported' % self.params['scale_function_type'])
            ...
            if self.params['movement'] == 'Down':
                ...
                indices = np.linspace(0, dat0.shape[0] - 1, self.params['video_len'], dtype='int')
            ...
            self.position_speed_limit_index = np.random.choice(len(self.params['position_speed_limits']))
            symbol.body.velocity *= np.random.uniform(
                *tuple(self.params['position_speed_limits'][self.position_speed_limit_index]))
        ...
        self._add_collision_handlers(interacting_symbols=self.params['interacting_symbols'])
    ...
    def _add_walls(self):
        ...
        for wall in walls:
            if self.params['movement'] == 'Vertical' or self.params['movement'] == 'Horizontal':
                wall.elasticity = 1
            else:
                wall.elasticity = 0
    ...
    def _symbol_wall_begin_handler(arbiter, space, data):
        ...
        data['mie']._publish_message(dict(step=data['mie']._step_count,
                                          type='hit_wall',
                                          meta=dict(symbol_id=symbol_id,
    ...
    def _symbol_symbol_overlap_begin_handler(arbiter, space, data):
        ...
        data['mie']._publish_message(dict(step=data['mie']._step_count,
                                          type='start_overlap',
                                          meta=dict(symbol_ids=overlapping_symbol_ids)))
    ...
    def _add_collision_handlers(self, interacting_symbols=False):
        ...
        Args:
            interacting_symbols: Boolean for whether symbols should bounce off each other
        """
        ...
        h = self._space.add_collision_handler(_COLLISION_TYPES['symbol'], _COLLISION_TYPES['symbol'])
        h.data['body_symbol_map'] = body_symbol_map
        h.data['mie'] = self
    ...
    def _step(self):
        ...
        for symbol in self.symbols:
            ...
            if self.params['movement'] == 'Circle' or \
               self.params['movement'] == 'Square' or \
               self.params['movement'] == 'Triangle' or \
               self.params['movement'] == 'Eight' or\
               self.params['movement'] == 'Infinite':
                sampled_velocity = self.dat1[self._step_count-1]
                symbol.body.velocity = sampled_velocity / np.linalg.norm(sampled_velocity)
                symbol.body.velocity *= np.random.uniform(
                    *tuple(self.params['position_speed_limits'][self.position_speed_limit_index]))
    ...
    def _render_pg(self):
        ...
        bg_sprite = pg.image.fromstring(self.background.tobytes(), self.background.size, self.background.mode)
        ...
        if self.debug_options['show_bounding_poly']:
            for symbol in self.symbols:
                ps = [p.rotated(symbol.body.angle) + symbol.body.position for p in symbol.shape.get_vertices()]
    ...
    def _publish_message(self, message):
        ...
        Args:
            message: Dict of information to publish
        """
    ...
    def add_subscriber(self, subscriber):
        ...
        Args:
            subscriber: An object with a callable "process_message" method
        """
