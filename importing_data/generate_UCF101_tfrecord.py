...
sys.path.append('/home/arenas/source_code/uNLC/src')
import run_full_uNLC_train as uNLC

tf.app.flags.DEFINE_string('videos_directory', '/ds2/ucf101/UCF-101/', 'Video data directory')
tf.app.flags.DEFINE_string('input_file', '/netscratch/arenas/dataset/ucf101_list/101_trainlist01.txt', 'Text file with (filename, label) pairs')
tf.app.flags.DEFINE_string('output_directory', '/netscratch/arenas/tfrecords/UCF-101-uNLC-tf-records/', 'Output data directory')
tf.app.flags.DEFINE_string('class_list', '/netscratch/arenas/dataset/ucf101_list/101_classlist.txt', 'File with the class names')
tf.app.flags.DEFINE_string('name', 'UCF-101-train01', 'Name for the subset')
tf.app.flags.DEFINE_integer('num_shards', 1400, 'Number of shards. Each job will process num_shards/num_jobs shards.')
tf.app.flags.DEFINE_integer('num_threads', 1, 'Number of threads within this job to preprocess the videos.')
tf.app.flags.DEFINE_integer('num_jobs', 10, 'How many jobs will process this dataset.')
...
tf.app.flags.DEFINE_integer('label_offset', 1, 'Offset for class IDs. Use 1 to avoid confusion with zero-padded elements')
...
def _convert_to_sequential_example(filename, video_buffer, mask_buffer, label, text, height, width, sequence_length, sample_length):
...
class VideoCoder(object):
    ...
    def _resize_video(self, seq_length, new_height, new_width):
        resized_video = tf.image.resize_bilinear(self._resize_video_data, [new_height, new_width], align_corners=False)
...
def _process_video(filename, coder):
    ...
    Args
        ...
        coder: instance of VideoCoder to provide TensorFlow image coding utils.
    Returns:
    ...
    # generate mask from uNLC
    frames, masks, new_length = uNLC.demo_images(filename, video)
    final_masks = np.expand_dims(masks, axis=3)

    encoded_frames_seq = []
    encoded_masks_seq = []
    for idx in range(new_length):
        encoded_frames_seq.append(coder.encode_frame(frames[idx, :, :, :]))
        encoded_masks_seq.append(coder.encode_mask(final_masks[idx, :, :, :]))

    return encoded_frames_seq, encoded_masks_seq, frame_h, frame_w, np.asscalar(np.array(new_length))
...
def _process_video_files_batch(coder, thread_index, ranges, name, filenames, texts, labels, num_shards,
    ...
    """
    assert not num_shards % num_jobs # Be sure we can split all the shards into each job
...
def _find_video_files(input_file, class_list_path, dataset_dir):
    ...
    for line in lines:
        ...
        label_id = class_list.index(label_name)
