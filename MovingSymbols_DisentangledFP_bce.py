# --------------------------------------------------------------------------- #
# --------------- Model 2: Disentangling appearance and motion -------------- #
# --------------------------------------------------------------------------- #
...
model_dir = '/netscratch/arenas/model/MovingSymbols2_Seen/MovingSymbols2_Seen_DisentangledFP_bce'
...
def get_estimator(hparams, run_config):
    ...
    def model_fn(features, labels, mode, params):
        ...
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
            a_feat_stop_gradient = tf.stop_gradient(a_feat)
            feat_dict = {'a_feat': a_feat, 'join_feat': mf.combine_features(a_feat_stop_gradient, m_feat)}
            output_decoder_dict = {}
            for key in feat_dict.keys():
                # Input tensor: (batch_size x (sequence_length-1))x(#units a_feat)
                input_decoder = tf.reshape(feat_dict[key], [FLG.batch_size*(FLG.seq_length-1), 1, 1, FLG.units])
                ...
                # Input tensor: (batch_size x (sequence_length-1))x64x64x(#last_filt_decoder)
                output_decoder = tf.reshape(deconv,
                                            ...
                                            name='output_decoder')
                output_decoder_dict.update({key: output_decoder})
        ...
        if mode == tf.estimator.ModeKeys.PREDICT:
            ...
            predictions = {'encoding': feat_dict['join_feat'], 'reconstruction': output_decoder_dict['join_feat']}
        ...
        with tf.variable_scope('loss'):
            a_binary_labels = tf.cast(bin_input_layer[:, FLG.seq_length-1:-2], tf.int64)
            a_loss = tf.losses.sparse_softmax_cross_entropy(labels=a_binary_labels,
                                                            logits=output_decoder_dict['a_feat'])

            join_binary_labels = tf.cast(bin_input_layer[:, FLG.seq_length:-1], tf.int64)
            join_loss = tf.losses.sparse_softmax_cross_entropy(labels=join_binary_labels,
                                                               logits=output_decoder_dict['join_feat'])

            loss = a_loss + join_loss + tf.losses.get_regularization_loss()
        ...
        predicted_frames_bin = tf.squeeze(tf.cast(tf.argmax(output_decoder_dict['join_feat'][0], axis=3) * 255, tf.float32))
        ...
        # Merge original_vs_predicted summaries to be shown in EVAL mode.
